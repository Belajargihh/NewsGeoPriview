document.addEventListener("DOMContentLoaded", () => {
  // ============================================================
  // 1. KONFIGURASI URL (Fix Path Double /api/api)
  // ============================================================
  ...
  const btnPrevHighlightMobile = document.getElementById("btn-prev-highlight-mobile");
  const btnNextHighlightMobile = document.getElementById("btn-next-highlight-mobile");
  const highlightPageIndicator = document.getElementById("highlight-page-indicator");
  ...
  // ============================================================
  // 2. FUNGSI EKSTRAK GAMBAR (Fix Path)
  // ============================================================
  ...
  // ============================================================
  // 3. FUNGSI HIGHLIGHT (Gunakan Endpoint Ringan /api/highlights)
  // ============================================================
  async function loadHighlights() {
    try {
      // Menggunakan method GET ke endpoint khusus highlight agar cepat & tidak timeout
      ...
      // Sesuaikan dengan format response endpoint highlights
      // Jika endpoint mengembalikan { highlights: [...] }
      const articles = data.highlights || data.artikel_terkait || [];

      if (articles.length > 0) {
        highlightArticles = articles.slice(0, 10);
  ...
  function renderHighlights() {
    ...
    visibleArticles.forEach((article) => {
      ...
      card.innerHTML = `
                  ...
                  `;
      // Fix: Pastikan URL ada sebelum pasang listener
      const url = article.url || "#";
      card.addEventListener("click", () => {
        if(url !== "#") handleArticleClick(url, article.judul);
      });
  ...
  // ============================================================
  // 4. FUNGSI PENCARIAN UTAMA (Fix Endpoint & Body)
  // ============================================================
  async function handleSearch() {
    ...
    try {
      // FIX: Gunakan endpoint /api/analyze dan body { query: ... }
  ...
  function displayPage(page) {
    ...
    paginatedArticles.forEach(async (article) => {
      ...
      const thumbnail = images[0] || "https://via.placeholder.com/150?text=No+Image";
  ...
  // ============================================================
  // 5. FUNGSI KLIK ARTIKEL / POPUP (Fix Endpoint & Body)
  // ============================================================
  async function handleArticleClick(url, title) {
    ...
    try {
      // FIX: Gunakan endpoint /api/analyze-url dan body { url: ... }
      ...
      if (images.length > 0) {
        imgHtml = `
        <div class="mb-4">
            <img src="${images[0]}" class="w-full rounded-xl shadow max-h-60 object-cover">
        </div>
    `;
      }
